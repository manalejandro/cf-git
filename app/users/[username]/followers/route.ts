...
export async function GET(request: NextRequest, { params }: { params: Promise<{ username: string }> }) {
  ...
  const followers = await getFollowers(db, actor.id);

  const collection = page === "true"
    ? buildOrderedCollectionPage(followersId, followers.map((f) => actorIRI(baseUrl, f.username)))
    : buildOrderedCollection(followersId, followers.length);
